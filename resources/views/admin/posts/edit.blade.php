    ...
    <div class="card">
        ...
        <form action="{{ route('admin.posts.update', $post) }}" method="post" enctype="multipart/form-data">
            ...
            <div class="h-52 rounded-md mb-4 object-cover overflow-hidden">
                ...
                @if ($post->image)
                    <img src="{{ asset('storage/' . $post->image) }}" alt="" class="h-full object-contain">
                @else
                    <img src="{{ asset('storage/post_images/default.jpg') }}" alt="" class="h-full object-contain">
                @endif
            </div>
            ...
            <div class="mb-4">
                ...
                <select name="user_id" id="user_id" class="input @error('user_id') ring-red-500 @enderror">
                    @foreach ($users as $user)
                        <option value="{{ $user->id }}" @selected(old('user_id', $post->user_id) == $user->id)>{{ $user->username }}</option>
                    @endforeach
                </select>
                @error('user_id')
                    <p class="error">{{ $message }}</p>
